...
public class TransferEngine {
    ...
    public void onChunkCompleted(TransferState state, int chunkIndex,
        ...
        boolean allDone = state.markChunkDone(chunkIndex);
        state.completedChunks.incrementAndGet();
    ...
    private void finalizeTransfer(TransferState state) {
        ...
        try {
            finalChecksum = computeOrderedHash(state);
        } catch (IOException e) {
        ...
        log.info("Transfer {} complete — finalChecksum={}", state.transferId, finalChecksum);

        if (state.progressStream != null) {
            try { state.progressStream.onCompleted(); } catch (Exception ignored) {}
        }

        cpPort.reportTransferFinalized(state.transferId, finalChecksum);
        activeTransfers.remove(state.transferId);
    }
     ...
     */
    private String computeOrderedHash(TransferState state) throws IOException {
